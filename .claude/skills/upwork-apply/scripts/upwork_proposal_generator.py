...
- NEVER use em dashes, en dashes, or hyphens as punctuation. Use periods or commas instead.
...
def generate_proposal(job: dict, client: anthropic.Anthropic) -> str:
    ...
    prompt = f"""Write a personalized project proposal for this Upwork job. Write as Jude - first person, conversational, direct.
...
[1 sentence mirroring their specific pain/need — show you actually read the job posting. Reference something specific from their description, not generic.]

I've built similar [specific thing relevant to this job]. [1-line proof, e.g. "Recently shipped a voice agent handling 500+ calls/day for a recruiting firm" or "Built an n8n pipeline processing 10K leads/week for a SaaS company". Make it relevant to THIS job's domain. Be specific and credible.]
...
I can start this week. Happy to walk you through my approach on a quick call.
...
- NEVER use em dashes, en dashes, or hyphens as punctuation (looks AI generated). Use periods, commas, or "and" instead.
...
SHEET_ID_FILE = '.tmp/current_sheet_id.txt'
...
def write_fresh_sheet(sheet_id: str, jobs: list[dict], sheets_service):
    """Write all data to a fresh sheet with headers + rows."""
    rows = [HEADERS] + [job_to_row(j) for j in jobs]
    sheets_service.spreadsheets().values().update(
        spreadsheetId=sheet_id,
        range=f'Jobs!A1:Q{len(rows)}',
...
def append_to_sheet(sheet_id: str, jobs: list[dict], sheets_service):
    """Append rows to existing sheet (no headers)."""
...
def main():
    ...
    parser.add_argument("--sheet-id", "-s", help="Google Sheet ID (overrides saved sheet ID)")
    parser.add_argument("--new-sheet", action="store_true", help="Force create a new sheet")
    ...
    # Resolve sheet ID: explicit > saved > new
    appending = False
    if args.sheet_id:
        sheet_id = args.sheet_id
        appending = True
    elif not args.new_sheet and os.path.exists(SHEET_ID_FILE):
