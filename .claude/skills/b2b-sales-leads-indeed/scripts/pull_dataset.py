"""
Shared helpers + standalone dataset puller for the B2B sales-hiring pipeline.

Scrapes Indeed JOB POSTINGS (via the `valig/indeed-jobs-scraper` actor) for
companies actively hiring B2B sales reps, then maps each posting into a 29-col
Google Sheet schema. `scrape_and_pull.py` imports the helpers here; this file
can also be run directly against an existing Apify dataset.
...
Column layout (29 cols):
  ...
  Derived     AB-AC: Role Type, Indeed URL
...
BATCH_SIZE = 10
TAB_NAME = "Leads"
SHEET_TITLE = "B2B Sales-Hiring Leads — Indeed"
...
HEADERS = [
    ...
    # Derived columns
    "Role Type",           # AB — Outside Sales / SDR / BDR / AE / B2B Sales (from job title)
    "Indeed URL",          # AC — https://www.indeed.com/viewjob?jk={Job_Id}
]


# Unambiguous leadership titles (never an individual contributor) — flagged so
# the sheet shows them and classify_companies.py can drop them.
_LEADERSHIP_RE = re.compile(
    r"\b(vp|v\.p\.|vice president|svp|evp|director|head of|chief|cro|cso|cmo|ceo|president)\b",
    re.IGNORECASE,
)


def classify_role_type(job_title):
    """Classify the B2B sales role from the job title."""
    t = (job_title or "").lower().strip()
    if not t:
        return ""
    if _LEADERSHIP_RE.search(t):
        return "Leadership"
    if "outside sales" in t:
        return "Outside Sales"
    if "sales development" in t or re.search(r"\bsdr\b", t):
        return "SDR"
    if "business development" in t or re.search(r"\bbdr\b", t):
        return "BDR"
...
def map_to_row(item):
    ...
    return [
        ...
        # Outreach (T-AA) — blank
        "", "", "", "", "", "", "", "",
        # Role Type (AB) — derived from Job Title
        classify_role_type(item.get("title", "")),
...
def main():
    parser = argparse.ArgumentParser(description="Pull Apify Indeed dataset → Google Sheet (B2B sales)")
    ...
    print("=== Pull Apify Dataset → Google Sheet (B2B Sales) ===\n")
