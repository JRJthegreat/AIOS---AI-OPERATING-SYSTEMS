...
Scrapes the National Coalition of Healthcare Recruiters job board
(nchcr.com) to build a lead list of healthcare recruitment firms. For each
job posting, the detail page exposes the posting firm's company name,
recruiter name, contact phone, and recruiter email — and the firm website is
...
def _next_page(page) -> bool:
    ...
    if not before:
        return False
    try:
        page.evaluate("__doPostBack('ctl00$ContentPlaceHolder1$lbNext','')")
    except Exception as e:
        ...
        if strong.get_text(strip=True).rstrip(":").lower() == label.lower():
            ...
            if label.lower() == "recruiter email":
                ...
                if a:
                    ...
                    m = _EMAIL_RE.search(txt) or _EMAIL_RE.search(a.get("href", ""))
...
def scrape(
    ...
    Args:
        job_type: "Both" | "Physicians" | "Non-Physicians"
        results_per_page: listings per results page (higher = fewer paginations)
        max_list_pages: cap on results pages crawled while enumerating firms
        max_firms: cap on unique firms / detail visits (0 = no cap)
        test: if True, crawl 1 listing page and at most 10 firms
    """
    from playwright.sync_api import sync_playwright

    max_pages = 1 if test else max_list_pages
    firm_cap = 10 if test else max_firms  # 0 = no cap
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            _set_results_per_page(listing, results_per_page)
            ...
            for page_num in range(1, max_pages + 1):
                ...
                if firm_cap and len(firms) >= firm_cap:
                    break
            ...
            if firm_cap:
                firm_items = firm_items[:firm_cap]
    ...
    log.info(f"[NCHCR] → {len(deduped)} unique firms ({with_web} with website)")
    return deduped
